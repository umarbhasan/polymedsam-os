...
# --- 1. Kvasir-SEG ID Data & Synthetic OOD Setup (For Training) ---
...
# Designate 15% of training data as Simulated OOD to learn the rejection margin
...
# Initialize the processor for MedSAM
processor = SamProcessor.from_pretrained("flaviagiammarino/medsam-vit-base")

def ood_transform(img_np):
    # Simulate artifacts: extreme high-frequency noise
...
class KvasirDatasetOSL(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Apply OOD transforms (Simulate artifacts/noise) to teach latent margin
        if self.is_ood and self.ood_transform:
            image_np = self.ood_transform(image_np)
            # Crucial: we provide a blank mask for noise so the decoder learns not to segment it
...
# --- 2. Authentic Kvasir-Instrument Setup (STRICTLY FOR EVALUATION) ---
...
# --- 3. Build Dataloaders ---
# Training Loaders (ID Polyps + Synthetic Noise)
...
def generate_instrument_grid(dataloader, num_samples=2, save_path='/kaggle/working/instrument_dataset_samples.pdf'):
    ...
    for batch in dataloader:
        ...
        for i in range(orig_imgs.shape[0]):
            # We want all samples from this loader, so no OOD skipping is needed
...
if os.path.exists(model_path):
    ...
    print(f"Warning: Could not find {model_path}. Make sure the file exists in the working directory.")
